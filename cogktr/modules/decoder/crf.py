...
class CRF(nn.Module):
    ...
    def _forward_alg_batch(self, feats_batch, sequence_lengths):
        ...
        for index, feats_b in enumerate(feats_transpose[1:]):
            emit_score = feats_b.view(batch_size, self.tagset_size, 1).expand(batch_size, self.tagset_size,
                                                                              self.tagset_size)
            next_tag_var = forward_var.view(batch_size, 1, self.tagset_size).expand(batch_size, self.tagset_size,
                                                                                    self.tagset_size) + emit_score + trans_score
    ...
    def _transition_potential_score(self, tags_batch):
        ...
        mask = Variable(torch.FloatTensor([0] + [1] * (max_len - 1))).expand_as(tags_batch).contiguous().view(-1).to(
            device)
