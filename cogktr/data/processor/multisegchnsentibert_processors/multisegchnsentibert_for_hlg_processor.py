...
from cogktr.enhancers.tagger.word_segmentation_tagger import WordSegmentationTagger
from transformers import AutoTokenizer

transformers.logging.set_verbosity_error()  # set transformers logging level


class MultisegchnsentibertForHLGProcessor(BaseProcessor):
    def __init__(self, max_token_len, vocab, plm):
        super().__init__()
        self.max_token_len = max_token_len
        self.vocab = vocab
        self.tagger = WordSegmentationTagger(tool="jieba")
        self.tokenizer = AutoTokenizer.from_pretrained(plm)

    def _process(self, data):
        datable = DataTable()
        print("Processing data...")
        for raw_sentence_str, label in tqdm(zip(data['raw_sentence_str'], data['label']), total=len(data['sentence'])):
            tagger_dict = self.tagger.tag(raw_sentence_str)
            tokenized_data = self.tokenizer.encode_plus(raw_sentence_str, add_special_tokens=True)
            sentence = tokenized_data["input_ids"]
            jieba_word_length = []
            jieba_word_length.append(1)
            jieba_word_length.extend(tagger_dict["knowledge"]["word_len"])
            jieba_word_length.append(1)
            ...
            for toolid, word_length_list in enumerate([jieba_word_length]):
                start_loc = 0
