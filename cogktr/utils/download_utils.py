...
class Downloader:
    ...
    def download_wikipedia2vec_knowledge(self, output_file):
        dataset_list = ["enwiki_20180420_win10_100d.pkl"]
        zip_name = "KNOWLEDGE_WIKIPEDIA2VEC.zip"
        self._download_data(output_file=output_file,
                            dataset_list=dataset_list,
                            zip_name=zip_name)

    def download_wikipedia_entity_description_knowledge(self, output_file):
        dataset_list = ["entity.jsonl"]
        zip_name = "KNOWLEDGE_WIKIPEDIA_ENTITY_DESCRIPTION.zip"
        self._download_data(output_file=output_file,
                            dataset_list=dataset_list,
                            zip_name=zip_name)
