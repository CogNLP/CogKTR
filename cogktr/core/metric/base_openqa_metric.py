...
class BaseOpenQAMetric(BaseMetric):
    def __init__(self):
        super(BaseOpenQAMetric, self).__init__()
    ...
    def get_metric(self, reset=True):
        ...
        for question_id,value in question_id2prediction.items():
            ...
            example = self.qas_id2example[str(question_id)+'-'+'0']
            ...
            question_id2em_score[question_id] = em_score
            question_id2f1_score[question_id] = f1_score

        eval_result = {
            "EM":np.mean(np.array([list(question_id2em_score.values())])),
            "F1":np.mean(np.array([list(question_id2f1_score.values())])),
        }
